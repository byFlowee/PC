...
/***
	reservar_memoria: reserva memoria para la cola en funcion de su capacidad y del tamaño de los elementos
***/
...
/*** 
	...
	varios de igual prioridad, saca el mas antiguo
***/
...
int quitar_de_la_cola (cola_t *cola, elem_t *elem, unsigned *prio)
{
	...
	*prio = cola->ptr_base[max].prio;
	...
	if (max == cola->primero)
		...
		aux = cola->ptr_base[i];
		/* Esta recolocacion esta acotada en el tiempo y no depende del tamano del
